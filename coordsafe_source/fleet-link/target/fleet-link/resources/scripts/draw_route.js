...
function genImageUrl (){
    ...
    var center = map.getCenter(), center_lat = (center.lat()).toFixed(6), center_lng = (center.lng()).toFixed(6);
    ...
    for(var i=0, l=display.length;i<l;i++){
        if(display[i].type=="route"){
            ...
            for(var j=0, l2=path.length;j<l2;j++){
                coordinates.push([path[j].lat(),path[j].lng()]);
            }
        }else if(display[i].type="line"){
            var path=display[i].drawing.getPath();
            var vts = path.getArray();
            if(coordinates.length==0){
                coordinates.push([vts[0].lat(),vts[0].lng()]);
            }
            coordinates.push([vts[vts.length-1].lat(),vts[vts.length-1].lng()]);
        }
    }
    ...
    //$.getJSON("http://192.168.100.12:8070/geoready/api/zone/60?key=test-1234qwer",function(data){
        ...
        if(load_data.type=="LineString"){
            ...
            for(var i=0, l=load_data.coordinates.length;i<l;i++){
                var point = new google.maps.LatLng(load_data.coordinates[i][0],load_data.coordinates[i][1]);
                path.push(point);
                bounds.extend(point);
            }
...
function saveMapImage(){
    ...
    $.getJSON("http://192.168.100.12:8070/geoready/api/zone/60?key=test-1234qwer",function(data){
        ...
        if(load_data.type=="LineString"){
            ...
            var center = bounds.getCenter(), center_lat = (center.lat()).toFixed(6), center_lng = (center.lng()).toFixed(6);
            var strokeColor = "blue";
            var url = "";
