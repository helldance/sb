...
<head>
    <meta charset="UTF-8"/>
    <!-- <link rel="stylesheet" href="css/onemap.css" type="text/css" media="screen" charset="utf-8"/> -->
    ...
    <script type='text/JavaScript' src='http://www.onemap.sg/API/JS/?accessKEY=dWQbM+0kRwxz6CeuJ3aY2wZud8lS1OMQNcKJdDPntTgo4/E5nwKfJybJ/VKLO66opvjpQQREoiHffMrfEYTVou8EsyTheowdaKthUA/Zogs1RVbRLRunT6PU3WMGTJrF|mv73ZvjFcSo=&v=2.8&type=full'></script>

    <script type="text/javascript" src="resources/assets/js/jquery-1.9.1.min.js"></script>
    <!-- <script type="text/javascript" src="resources/assets/js/jquery-ui-1.10.3.custom.min.js"></script> -->
    <script src="resources/scripts/jquery-ui-1.10.3.custom.min.withDTpicker.js"></script>
    <script type="text/javascript" src="resources/scripts/jquery-ui-timepicker-addon.js"></script>	
    <script type="text/javascript" src="resources/scripts/chart/playback.js"></script>
    <script type="text/javascript" src="resources/scripts/onemap/onemap.js"></script>
    <script type="text/javascript" src="resources/scripts/onemap/onemap_classy.js"></script>
    <script type="text/javascript" src="resources/scripts/onemap/spatial-reference-converter.js"></script>
    <script type="text/javascript" src="resources/scripts/chart/raphael-min.js"></script>
    <script type="text/javascript" src="resources/scripts/chart/g.raphael-min.js"></script>
    <script type="text/javascript" src="resources/scripts/chart/g.line-min.js"></script>
	...
	<script type="text/javascript" src="resources/assets/js/jquery.dataTables.bootstrap.js"></script>
...
<body>
	...
	$(document).ready(function() {
		...
		/* var centerPoint="23860.597,32718.861";
		var levelNumber=8;
		var OneMap = new GetOneMap('map','LL',{level:levelNumber,center:centerPoint}); */
		var map;
		...
		var markerOptions = {"url":locatorImage, "width":32, "height":26};
		...
		// add lta traffic info
		//var layer = new esri.layers.ArcGISDynamicMapServiceLayer("http://www.onemap.sg/arcgis/rest/services/LTATrafficDensity/MapServer/3?f=pjson");
			...
			var req = "/fleet-link/vehicle/company/" + companyId;
	                ...
	                $.getJSON(req, function(data) {     
                        ...
                        $.each(vehicles, function (i, item) {    
                            ...
                            var svy21_result = cv.computeSVY21(item.latLng.latitude, item.latLng.longitude);     
                            
                            if (item.status === "MOVING"){
                            	markerOptions = {"url":locatorImage, "width":32, "height":26};
                            	amt_active += 1;
                            }
                            else if (item.status === "SPEEDING"){
                            	markerOptions = {"url":locatorImage_Speeding, "width":32, "height":26};
                            	amt_active += 1;
                            }
                            else if (item.status === "STOPPED") {
                            	markerOptions = {"url":locatorImage_Stopped, "width":32, "height":26};
                            	amt_stop += 1;
                            }
                            else {
                            	markerOptions = {"url":locatorImage_Idle, "width":32, "height":26};
                            	amt_pause += 1;
                            } 
                            
                            var vehicle_marker = new Marker(svy21_result.E, svy21_result.N, markerOptions);
                            var title = item.licensePalette;
                            var info = "<div style='background-color: #ABC'><div><b>" + item.vehicleName +  "</b>"  + " - <i>" + item.licensePalette + "</i><span class='pull-right' style='margin-right: 24px'>" + item.status 
                            	+ "</span></div><hr><div style='width: 100%'><span><i class='icon-time icon-large'></i> &nbsp; " + dateFormat(new Date(item.lastUpdate), "dddd, mmmm dS, yyyy, h:MM:ss TT") + "</span></div>"; 
                            ...
                            createLocatorMarker(vehicle_marker, title, info, item.status);
        ...
        function createLocatorMarker(marker, title, info, status) {
        	//var marker;
        	...
        	var infoTemplate = new esri.InfoTemplate(title,info);
        ...
        function drawPath(data){
            ...
            for(var i=0;i<data.length;i++){
                ...
                var polyOptions = {"lineStyle":esri.symbol.SimpleLineSymbol.STYLE_SOLID, "lineColor": new dojo.Color(colors[i]), "lineWidth": 4};
                ...
                for(var j=0;j<len;j++){
                	...
                	var svy21_result = cv.computeSVY21(tempData[j].lat,tempData[j].lng);
                ...
                var _poly = new Polyline(path,polyOptions);
                _poly.setId(i);
                _poly.setMap(map);  
                ...
                //Add numbered marker
                var svy21_result = cv.computeSVY21(tempData[Math.floor(len/2)].lng, tempData[Math.floor(len/2)].lng); 
                var number_icon = "/fleet-link/resources/numbered_marker/number_" + i + ".png";
                var num_options = {"url":number_icon, "width":32, "height":26};
                var num_marker = new Marker(svy21_result.E, svy21_result.N, markerOptions);
                
                num_marker.setMap(map);

                trip_markers.push(num_marker);
        ...
        function getSelectedLocator(sel){
        	...
        	curSelectedLocator = $("#sel_locator").val();
        	...
        	<div id="info_tabs">
        		<form>
        			...
        			<div class="input-prepend">
        				...
        				<select id="sel_group" onchange="getSelectedGroup(this)">
        				</select>
        			</div>

        			<div class="input-prepend">
        				<span class="add-on span4">Vehicle</span>
        				<select id="sel_locator" onchange="getSelectedLocator(this)">
        				</select>
        			</div>
        		</fieldset>
        	</form>
        	...
        	<div id="vehicle" class="tab-pane">
        		...
        		<!-- <div class="widget-box transparent">
        			<div class="widget-header">
        				<h4 class="lighter"><i class="icon-map-marker"></i>&nbsp;STATUS</h4>
