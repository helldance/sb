...
function createQuery(){
	...
	if(type=="weekly"){
		...
		firstDate = new Date(2013,value,1);
		lastDate = new Date(2013,value+1,0);
	}
...
function drawAPIKeyCounterChart(data){
	...
	//Drawing chart title
	r.counter.text(w/8,4,"# of Request").attr("font-size",16);
...
function drawAPIKeyResourceChart(data){
	...
	for(var i=0, l=data.length; i<l; i++){
		var pattern = /api\/(.+?)\//g;
		var match = pattern.exec(data[i].resource)[1];
